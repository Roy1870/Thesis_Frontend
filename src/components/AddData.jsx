...
const EditFarmer = ({ farmer, onClose, colors }) => {
  ...
  const handleDeleteLivestock = async (recordId) => {
    try {
      console.log("Component - Record ID received:", recordId);

      // If recordId is undefined, try to find it in other properties
      if (!recordId) {
  ...
  const livestockColumns = [
    ...
    },
    {
      ...
      width: 120,
      render: (_, record) => {
        console.log("Livestock record:", record); // Add this line to debug
        return (
          <Space size="small">
            <Button
              type="text"
            ...
            <Popconfirm
              ...
              onConfirm={() => {
                ...
                // Try to find the ID in different possible properties
                const recordId = record.id || record._id || record.livestock_id;
                console.log("Using record ID:", recordId);
                handleDeleteLivestock(recordId);
              }}
              okText="Yes"
              cancelText="No"
              okButtonProps={{
        ...
        >
          {livestockLoading ? (
            ...
            <Table
              ...
              rowKey={(record) => {
                // Try to find a unique identifier in the record
                const id = record.id || record._id || record.livestock_id;
                if (id) {
                  return id;
                }
                // If no ID is found, create a fallback unique key
                return `${record.animal_type}-${
                  record.subcategory
                }-${Math.random()}`;
              }}
              pagination={false}
